...
| Search analytics (`SearchQueryLog`, `SearchResultClick`) | query text is redacted before it is written (`search.analytics.redactionPatterns`); raw rows rolled up daily into `SearchQuerySummary` and purged after `search.analytics.retentionDays` (default 90); no IP address or session key is stored | `job_Maint_RollupSearchLogs.sql` reads the setting from `SiteSetting` on every run. Readers: SiteAdmin/SystemAdmin in the API; `vacms_readonly` is denied the raw tables. Data category and PTA status: `docs/SECURITY_CONTROLS.md` § 6.1 (#175) |
