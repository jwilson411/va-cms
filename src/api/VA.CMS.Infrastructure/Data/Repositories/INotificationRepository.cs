...
/// All methods call EXEC usp_Notification_* stored procedures (V039, V040).
/// </summary>
public interface INotificationRepository
{
    /// <summary>
    /// Fan one workflow event out to its recipients (see V040 for the recipient rules).
    /// Returns the notifications created, one per recipient with their email address, so the
    /// caller can send the matching emails (issue #39) — empty when nobody but the actor qualifies.
    /// Calls usp_Notification_CreateForWorkflowEvent.
    /// </summary>
    Task<IReadOnlyList<NotificationRecipient>> CreateForWorkflowEventAsync(long contentEntryId, string eventType, long actorId, string? comment = null);
